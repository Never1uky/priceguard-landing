    ...
    <header className="sticky top-0 z-40 border-b border-line/70 bg-surface/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 sm:px-6">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="rounded-full bg-ink px-3.5 py-2 text-xs font-semibold text-white no-underline transition hover:bg-ink-soft sm:text-sm"
        >
          Установить
        </a>
      </div>
      ...
      >
        {links.map((link) => (
          <NavLink
            ...
            className={({ isActive }) =>
              [
                'shrink-0 rounded-full px-3 py-1.5 text-xs font-medium no-underline',
